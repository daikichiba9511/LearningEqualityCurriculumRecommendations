...
class MNRCollator:
    ...
    def __call__(self, features):
        ...
        input_ids_topic = [
            x["input_ids_a"] + [pad_token_id] * (min(longest_topic, self.max_length) - len(x["input_ids_a"]))
            for x in features
        ]
        attention_mask_topic = [
            x["attention_mask_a"] + [0] * (min(longest_topic, self.max_length) - len(x["attention_mask_a"]))
            for x in features
        ]

        input_ids_content = [
            x["input_ids_b"] + [pad_token_id] * (min(longest_content, self.max_length) - len(x["input_ids_b"]))
            for x in features
        ]
        attention_mask_content = [
            x["attention_mask_b"] + [0] * (min(longest_content, self.max_length) - len(x["attention_mask_b"]))
            for x in features
        ]
