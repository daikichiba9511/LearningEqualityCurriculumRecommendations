...
# =========================================================================================
def read_data(cfg):
    train = pd.read_csv(f"{ROOT}/output/lecr-unsupervised-train-set/train.csv")
...
def build_training_set(topics, content, cfg):
    ...
    items = [(k, pred) for k in range(len(topics)) for pred in predictions]
...
def main() -> None:
    ...
    [train_and_evaluate_one_fold(train, correlations, fold, CFG) for fold in range(CFG.n_folds)]
